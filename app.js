...
function sendRegOtp() {
  ...
  const el = document.getElementById('regOtp');
  if (el) 
    el.value = otp;
}
...
function completeRegistration() {
  ...
  if (phone !== pendingPhone) { 
    alert('Phone does not match the one used to request OTP'); 
    return; 
  }
  if (otp !== pendingOtp) { 
    alert('Invalid OTP'); 
...
function sendLoginOtp() {
  ...
  if (!registered) {
     alert('No registered phone found. Please register first.'); 
     return; 
    }
  if (phone !== registered) { 
    alert('This phone is not registered. Use the registered phone number.'); 
    return; 
  }
  const otp = Math.floor(1000 + Math.random() * 9000).toString();
  ...
  const el = document.getElementById('loginOtp'); 
  if (el) 
    el.value = otp;
}
...
function completeLogin() {
  ...
  if (phone !== pendingPhone) { 
    alert('Phone does not match OTP request. Click Send OTP again.'); 
    return; 
  }
  if (otp !== pendingOtp) { 
    alert('Invalid OTP'); 
